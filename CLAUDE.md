...
```
`.env` holds `VITE_SUPABASE_URL` + `VITE_SUPABASE_KEY` (publishable). Supabase project ref is embedded in the key's JWT.
...
- Google login: `signInWithGoogle()` in AuthProvider. Requires enabling the Google provider in Supabase Dashboard (Auth → Sign In / Providers) + Google Cloud OAuth client with redirect `https://YOUR-PROJECT.supabase.co/auth/v1/callback`, and adding the app origin to Auth → URL Configuration → Redirect URLs. Trigger `handle_new_user` pulls full_name/avatar from OAuth metadata.
